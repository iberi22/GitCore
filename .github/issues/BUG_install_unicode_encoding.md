---
title: "BUG: install.ps1 fails when downloaded remotely due to Unicode encoding (.✨)"
labels:
  - bug
  - critical
  - cli
...
## Description
When running the remote installer via `irm ... | iex`, the PowerShell parser fails with syntax errors because the `.✨` directory name contains a Unicode emoji that gets corrupted during the HTTP download/piped execution.
...
### Option A: Replace .✨ with ASCII-safe name
Replace `.✨` with `.ai-core` or similar ASCII-only directory name across the entire protocol.
...
Use PowerShell's escaped Unicode syntax: `".`u{2728}"` instead of literal `.✨`.
