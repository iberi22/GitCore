...
**Problema:** El script aún referencia la carpeta local `agents-flows-recipes` y rutas `.ai/` en lugar de `.✨/`:

```powershell
# Línea 25-27 - INCORRECTO:
$RecipeRepo = "agents-flows-recipes"      # ❌ Carpeta ya no existe
$ContextFile = ".ai/CURRENT_CONTEXT.md"   # ❌ Debería ser .✨/
$IndexFile = ".ai/AGENT_INDEX.md"         # ❌ Debería ser .✨/
```
...
Pero internamente dice `.ai/ARCHITECTURE.md` en lugar de `.✨/ARCHITECTURE.md`.
...
### 8. Falta `.gitignore` para `.✨/CURRENT_CONTEXT.md`
...
|------------|--------|-------|
| `.✨/AGENT_INDEX.md` | ✅ | 33 recetas indexadas correctamente |
...
# Issue 2: Migrar referencias .ai → .✨
gh issue create --title "refactor: Replace all .ai/ references with .✨/" \
  --body "Múltiples archivos tienen referencias a .ai/ que debe ser .✨/" \
  --label "refactor"
